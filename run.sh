...
_cleanup() {
    ...
    sudo ip link del `ip link |grep "@${LAN_PARENT}" |awk -F':' '{print $2}' |sed 's/@/ link /'` || true
    sudo ip link del `ip link |grep "@${WAN_PARENT}" |awk -F':' '{print $2}' |sed 's/@/ link /'` || true
    echo -ne "* finished"
}
...
_prepare_network() {
    case $LAN_DRIVER in
        macvlan)
            ...
            sudo ip address add ${LAN_ADDR%.[0-9]*}.2/${LAN_NETMASK} broadcast ${LAN_ADDR%.[0-9]*}.255 dev $LAN_IFACE
    ...
    case $WAN_DRIVER in
        ...
        macvlan)
            ...
            sudo ip address add ${WAN_GATEWAY}/${WAN_NETMASK} broadcast ${WAN_ADDR%.[0-9]*}.255 dev $WAN_IFACE
            sudo ip route add $WAN_SUBNET dev $WAN_IFACE
            docker exec -it $CONTAINER sh -c "
                    uci -q set network.wan.proto='static'
                    uci -q set network.wan.ipaddr=${WAN_ADDR}
